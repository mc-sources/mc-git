...
pub fn stage_unstage(repo: &dyn GitRepository, tr: &TestRepo) {
    ...
    let a_entry = status.iter().find(|s| s.path == "a.txt").expect("a.txt in status");
    assert_ne!(a_entry.staged, FileStatusKind::Clean, "a.txt should be staged");

    repo.unstage_file("a.txt").expect("unstage a.txt");
    let status = repo.get_status().expect("get_status after unstage");
    let a_entry = status.iter().find(|s| s.path == "a.txt").expect("a.txt in status");
    assert_ne!(a_entry.staged, FileStatusKind::Added, "a.txt should not be staged as Added after unstage");

    repo.stage_all().expect("stage_all");
    let status = repo.get_status().expect("get_status after stage_all");
    for entry in &status {
        if entry.path != "init.txt" {
            assert_ne!(entry.staged, FileStatusKind::Clean, "{} should be staged", entry.path);
        }
    }
    ...
    for entry in &status {
        assert_ne!(entry.staged, FileStatusKind::Added, "{} should not be staged as Added after unstage_all", entry.path);
    }
}
...
pub fn branch_lifecycle(repo: &dyn GitRepository, tr: &TestRepo) {
    ...
    let feat = repo.create_branch("feature-x", &init_commit.oid).expect("create_branch");
    ...
    assert!(names.contains(&"feature-x"), "feature-x should appear in branch list");

    repo.checkout_branch("feature-x").expect("checkout feature-x");
    let branches = repo.list_branches(None).expect("list after checkout");
    let feat = branches.iter().find(|b| b.name == "feature-x").expect("find feature-x");
    ...
    repo.create_commit("feature commit").expect("commit on feature");

    let renamed = repo.rename_branch("feature-x", "feature-y").expect("rename_branch");
    assert_eq!(renamed.name, "feature-y");

    let branches = repo.list_branches(None).expect("list branches");
    let initial_branch = branches.iter()
        .find(|b| b.name != "feature-y" && !b.is_remote)
        .expect("should have another branch besides feature-y");
    repo.checkout_branch(&initial_branch.name).expect("checkout initial");

    repo.delete_branch("feature-y", true).expect("delete_branch");
...
pub fn git_config(repo: &dyn GitRepository, _tr: &TestRepo) {
    repo.set_git_config("test.integration", "hello", false).expect("set_git_config");
    let value = repo.get_git_config("test.integration", false).expect("get_git_config");
    assert_eq!(value, Some("hello".to_string()));

    let missing = repo.get_git_config("test.nonexistent", false).expect("get missing key");
...
pub fn remote_local(repo: &dyn GitRepository, tr: &mut TestRepo) {
    ...
    repo.push_remote("origin", &branch_name).expect("push_remote");
...
pub fn merge_clean(repo: &dyn GitRepository, tr: &TestRepo) {
    ...
    repo.create_branch("feature-merge", &init.oid).expect("create feature-merge");
    repo.checkout_branch("feature-merge").expect("checkout feature-merge");
    ...
    repo.create_commit("feature commit").expect("feature commit");

    // Find main branch name and checkout
    let branches = repo.list_branches(None).expect("list_branches");
    let main_branch = branches.iter()
    ...
    // Merge with no-ff
    let status = repo.merge_branch("feature-merge", true).expect("merge_branch");
    assert!(!status.has_conflicts, "clean merge should not have conflicts");

    // Log should show the merge commit
    let log = repo.get_log(10, 0, None, None).expect("get_log");
    assert!(log.len() >= 3, "should have at least 3 commits (init + feature + merge)");
    // The merge commit is the most recent
    assert!(log[0].parent_oids.len() >= 2, "merge commit should have >= 2 parents");
...
pub fn merge_conflict_resolve(repo: &dyn GitRepository, tr: &TestRepo) {
    ...
    // Create and checkout feature branch
    repo.create_branch("feature-conflict", &init.oid).expect("create feature-conflict");
    repo.checkout_branch("feature-conflict").expect("checkout feature-conflict");
    ...
    repo.create_commit("feature change").expect("feature commit");
    ...
    let status = repo.merge_branch("feature-conflict", true).expect("merge_branch");
    ...
    repo.create_commit("merge: resolve conflict").expect("merge commit");

    // State should be clean
    let state = repo.get_repository_state().expect("get_repository_state after resolve");
    assert_eq!(state, "clean", "state should be clean after conflict resolution");
}
...
pub fn merge_abort(repo: &dyn GitRepository, tr: &TestRepo) {
    ...
    // Create and checkout feature branch
    repo.create_branch("feature-abort", &init.oid).expect("create feature-abort");
    repo.checkout_branch("feature-abort").expect("checkout feature-abort");
    ...
    // Merge — should conflict
    let status = repo.merge_branch("feature-abort", true).expect("merge_branch");
    assert!(status.has_conflicts, "merge should have conflicts");
    ...
    // State should be clean
    let state = repo.get_repository_state().expect("get_repository_state after abort");
    ...
    assert_eq!(content, "main version", "file should be restored to main version after abort");
}
...
pub fn diff_file(repo: &dyn GitRepository, tr: &TestRepo) {
    ...
    let diff = repo.get_file_diff("diff-target.txt", false, false).expect("get_file_diff unstaged");
    ...
    let diff = repo.get_file_diff("diff-target.txt", true, false).expect("get_file_diff staged");
...
pub fn remote_network_fetch(repo: &dyn GitRepository, _tr: &TestRepo, env_var: &str) {
    ...
    repo.fetch_remote("test-remote").expect("fetch_remote should succeed with valid credentials");
...
pub fn log_and_detail(repo: &dyn GitRepository, tr: &TestRepo) {
    ...
    let detail = repo.get_commit_detail(&log[0].oid).expect("get_commit_detail");
    assert_eq!(detail.summary, "commit 3");
    assert!(!detail.changed_files.is_empty(), "should list changed files");
}
